...
function FavCard({ r, onRemove }: { r: Restaurante; onRemove: (id: number) => void }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["restaurante", r.id] }),
  });
  ...
  return (
    <article className="bg-white rounded-2xl border border-stone-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 overflow-hidden flex flex-col">
...
export default function FavoritosPage() {
  ...
  const restaurantes = queries
    .filter(q => q.data)
    .map(q => q.data as Restaurante);
    ...
    <main className="max-w-5xl mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-stone-500 text-sm">
          {ids.length === 0
            ? "Aun no has guardado ningun restaurante."
      ...
      {!isLoading && restaurantes.length > 0 && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
            {restaurantes.map(r => (
              <FavCard key={r.id} r={r} onRemove={handleRemove} />
            ))}
          </div>
          <div className="text-center">
            <Link to="/"
              className="inline-block border border-stone-200 hover:border-orange-300 text-slate-600 hover:text-orange-600 font-semibold px-6 py-2.5 rounded-xl transition-colors text-sm bg-white">
              Seguir explorando
            </Link>
          </div>
        </>
