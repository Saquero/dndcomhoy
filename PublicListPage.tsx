...
function RestauranteCard({ r, distancia }: { r: Restaurante; distancia?: number }) {
    ...
    <article className="group bg-white rounded-2xl border border-stone-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 overflow-hidden flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="mt-auto flex gap-2">
          ...
          <a href={mapsUrl} target="_blank" rel="noopener noreferrer" title="Como llegar"
...
export default function PublicListPage() {
  ...
  const clearAll = () => { setActiveFlags({}); setSearch(""); setSearchInput(""); setCiudad(""); setPage(1); setNearMe(false); };
  ...
  const handleNearMe = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setGeoError("No pudimos obtener tu ubicacion. Comprueba los permisos del navegador.");
        setGeoLoading(false);
      }
    );
  };
  ...
  const hasFilters = Object.keys(activeFlags).length > 0 || search || ciudad || nearMe;
  ...
  if (nearMe && userPos) {
    restaurantes = [...restaurantes]
      ...
      .sort((a, b) => {
        const da = distanciaKm(userPos.lat, userPos.lon, a.latitud!, a.longitud!);
        const db = distanciaKm(userPos.lat, userPos.lon, b.latitud!, b.longitud!);
        return da - db;
      })
      .slice(0, 12);
  }
    ...
    <main className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-3 mb-5">
        <button onClick={handleNearMe} disabled={geoLoading}
          className={`flex items-center gap-2 text-sm font-medium px-4 py-2 rounded-xl border transition-all ${
            nearMe ? "bg-blue-500 text-white border-blue-500" : "bg-white text-slate-600 border-stone-200 hover:border-blue-300 hover:text-blue-600"
          }`}>
          <IconLocation />
          {geoLoading ? "Buscando..." : nearMe ? "Cerca de mi (activo)" : "Cerca de mi"}
        </button>
        {geoError && <p className="text-xs text-red-500">{geoError}</p>}
        {nearMe && userPos && <p className="text-xs text-stone-400">Mostrando los mas cercanos a tu ubicacion</p>}
      </div>
          ...
          return (
            <button key={key} onClick={() => toggleFlag(key)}
              className={`text-xs font-medium px-3 py-1.5 rounded-full border transition-all ${
                isOn ? "bg-orange-500 text-white border-orange-500 shadow-sm" : "bg-white text-slate-600 border-stone-200 hover:border-orange-300 hover:text-orange-600"
              }`}>
              {label}
            </button>
          );
      ...
      {!isLoading && !isError && restaurantes.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 px-6 bg-white rounded-3xl border border-stone-100">
          <svg className="w-20 h-20 mb-5 text-stone-100" fill="none" stroke="currentColor" strokeWidth="1" viewBox="0 0 80 80">
          ...
          <h3 className="text-lg font-bold text-slate-700 mb-2 text-center">No hemos encontrado ningun sitio con esos filtros</h3>
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            <button onClick={clearAll} className="bg-orange-500 hover:bg-orange-600 text-white text-sm font-semibold px-5 py-2 rounded-xl transition-colors">
              Limpiar filtros
            </button>
            <Link to="/sugerir" className="border border-stone-200 bg-white hover:border-orange-200 text-slate-600 text-sm font-medium px-5 py-2 rounded-xl transition-colors">
        ...
        <>
          <div className="flex items-center justify-between mb-5">
            <p className="text-sm text-stone-400">
              {nearMe ? `${restaurantes.length} mas cercanos` : `${meta?.total ?? 0} restaurante${(meta?.total ?? 0) !== 1 ? "s" : ""}${hasFilters ? " con estos filtros" : ""}`}
            </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {restaurantes.map(r => (
              <RestauranteCard
                key={r.id}
                r={r}
                distancia={nearMe && userPos && r.latitud && r.longitud
                  ? distanciaKm(userPos.lat, userPos.lon, r.latitud, r.longitud)
                  : undefined}
              />
            ))}
          </div>
