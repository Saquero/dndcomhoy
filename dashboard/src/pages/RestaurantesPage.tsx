...
function buildParams(
  ...
  if (onlyWithoutPhotos) p.onlyWithoutPhotos = true;
...
function getPhotoCount(r: Restaurante) {
  return Array.isArray(r.imagenes) ? r.imagenes.length : 0;
}
...
export default function RestaurantesPage() {
  ...
  const [onlyWithoutPhotos, setOnlyWithoutPhotos] = useState(false);
  ...
  const { data, isLoading, isError, refetch } =
    useQuery<RestaurantesResponse>({
      queryKey: ["restaurantes-admin", filters, page, onlyWithoutPhotos],
      queryFn: () =>
        getRestaurantes(buildParams(filters, page, onlyWithoutPhotos)),
      staleTime: 15_000,
    });
  ...
  const ciudades = [
    ...new Set(restaurantes.map((r) => r.ciudad).filter(Boolean)),
  ].sort();

  const withPhotosCount = restaurantes.filter((r) => getPhotoCount(r) > 0).length;
  const withoutPhotosCount = restaurantes.length - withPhotosCount;
  ...
  const handleDelete = (r: Restaurante) => {
    if (!confirm(`Eliminar "${r.nombre}"? Esta accion no se puede deshacer.`)) {
      return;
    }
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="rounded-2xl border bg-white shadow-sm px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <p className="text-sm font-bold text-slate-700">
            Fotos de restaurantes
          </p>
          <p className="text-xs text-slate-400">
            {withPhotosCount} con foto · {withoutPhotosCount} sin foto en esta página
          </p>
        </div>
        ...
        <label className="inline-flex items-center gap-2 text-sm font-medium text-slate-600 cursor-pointer">
          <input
            ...
            onChange={(e) => {
              setOnlyWithoutPhotos(e.target.checked);
              setPage(1);
            }}
                ...
                return (
                  <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center hidden md:table-cell">
                      {photoCount > 0 ? (
                        <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 border border-emerald-100 text-xs font-bold px-2 py-0.5 rounded-full">
                          📸 {photoCount}
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-700 border border-amber-100 text-xs font-bold px-2 py-0.5 rounded-full">
                          ⚠️ Sin foto
                        </span>
                      )}
                    </td>
