# ============================================================
# instalar_mejoras_public.ps1
# Instala las nuevas features en el frontend publico:
# - Pagina Mis Favoritos
# - Boton Compartir en detalle
# - Formulario de correccion en detalle
# - SEO dinamico por restaurante
# - Boton Cerca de mi con geolocalizacion
# - Nav actualizado con contador de favoritos
...
Write-Host "  DCH - Instalando nuevas features en public/" -ForegroundColor Cyan
Write-Host "  Raiz: $ROOT" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""

if (-not (Test-Path "$ROOT\public")) {
    Write-Host "ERROR: No se encuentra public/ en $ROOT" -ForegroundColor Red
    exit 1
}
...
function Write-File {
    ...
    if (Test-Path $abs) {
        ...
        Copy-Item $abs "$abs.backup_$ts" -Force
        Write-Host "   Backup: $(Split-Path $abs -Leaf).backup_$ts" -ForegroundColor DarkGray
    }
...
# ============================================================
# 1. App.tsx — nueva ruta /mis-favoritos
# ============================================================
...
Write-File "public\src\App.tsx" ($appLines -join "`n")

# ============================================================
# 2. SiteHeader.tsx — contador de favoritos en nav
# ============================================================
Write-Host ">> [2/5] SiteHeader.tsx..." -ForegroundColor Yellow
...
Write-File "public\src\shared\SiteHeader.tsx" ($headerLines -join "`n")

# ============================================================
# 3. FavoritosPage.tsx — pagina nueva
# ============================================================
Write-Host ">> [3/5] FavoritosPage.tsx (archivo nuevo)..." -ForegroundColor Yellow
...
Write-File "public\src\pages\FavoritosPage.tsx" ($favLines -join "`n")

# ============================================================
# 4. PublicDetailPage.tsx — compartir + correccion + SEO
# ============================================================
Write-Host ">> [4/5] PublicDetailPage.tsx..." -ForegroundColor Yellow

# Leemos el archivo desde outputs si existe, sino usamos el de new_files
$detailSrc = Join-Path $ROOT "public\src\pages\PublicDetailPage.tsx"
# Usamos el archivo que ya generamos anteriormente
$detailFile = "/home/claude/new_files/PublicDetailPage.tsx"
if (Test-Path $detailFile) {
    $detailContent = [System.IO.File]::ReadAllText($detailFile, $enc)
    Write-File "public\src\pages\PublicDetailPage.tsx" $detailContent
} else {
    Write-Host "   AVISO: PublicDetailPage.tsx fuente no encontrado. Descargalo manualmente." -ForegroundColor Yellow
}

# ============================================================
# 5. PublicListPage.tsx — cerca de mi + distancia en cards
# ============================================================
Write-Host ">> [5/5] PublicListPage.tsx..." -ForegroundColor Yellow

$listFile = "/home/claude/new_files/PublicListPage.tsx"
if (Test-Path $listFile) {
    $listContent = [System.IO.File]::ReadAllText($listFile, $enc)
    Write-File "public\src\pages\PublicListPage.tsx" $listContent
} else {
    Write-Host "   AVISO: PublicListPage.tsx fuente no encontrado. Descargalo manualmente." -ForegroundColor Yellow
}
...
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "Features instaladas:" -ForegroundColor White
Write-Host "  /mis-favoritos    -> nueva pagina con restaurantes guardados" -ForegroundColor Gray
Write-Host "  Nav               -> contador rojo de favoritos en header" -ForegroundColor Gray
Write-Host "  Detalle           -> boton Compartir (copia URL o menu nativo movil)" -ForegroundColor Gray
Write-Host "  Detalle           -> formulario Sugerir correccion al final" -ForegroundColor Gray
Write-Host "  Detalle           -> SEO dinamico (title cambia por restaurante)" -ForegroundColor Gray
Write-Host "  Listado           -> boton Cerca de mi con geolocalizacion" -ForegroundColor Gray
Write-Host "  Listado           -> distancia en km/m en cada card cuando activo" -ForegroundColor Gray
Write-Host ""
Write-Host "Vite recarga solo si tienes npm run dev activo." -ForegroundColor White
Write-Host ""
Write-Host "Para hacer build de produccion:" -ForegroundColor Yellow
Write-Host "  cd public && npm run build" -ForegroundColor Cyan
Write-Host ""
